...
const FinalLoading = ({ navigation }) => {
    const route = useRoute();
    const { image, billName, image2 } = route.params || {};
    console.log(image, billName, image2);
    const [show, setShow] = useState(false);
    const handleDone = (() => {
        console.log(image, billName, image2);
        const formData = new FormData();
        formData.append('billNumber', billName);
        formData.append('billPicture', {
            uri: image,
            type: 'image/jpeg',
            name: 'bill_image.jpg',
        });
        ...
        try {
            axios.post(
                'https://www.lohawalla.com/api/v1/loadingBill/addBill',
                formData,
                {
                    headers: {
