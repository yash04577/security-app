...
const Final = ({ navigation }) => {
    ...
    const apiUrl = `${RBASE_URL}/employee/assign-qr`;
    ...
    const handleAlertConfirm = async () => {
        ...
        setShowAlert(false);
        try {
            await axios.patch(`${apiUrl}/${employeeId}`, { data: total.requestBody.data });
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                <Text style={{ color: 'black', fontSize: 30, marginBottom: 30 }}>Assign this QR to:{total?.selectedEmployee?.name}</Text>
                {/* Box Card */}
                <View style={styles.boxCard}>
                    {/* Render ProfilePic */}
                    <Image style={styles.profilePic} source={{ uri: total.selectedEmployee.Profile.uri }} />
                    {/* Render Name */}
                    <Text style={styles.name}>Name: {total?.selectedEmployee?.name}</Text>
                    {/* Render Department Name */}
                    <Text style={styles.departmentName}>Department Name: {total?.selectedEmployee?.dName}</Text>
                    {/* Render Department ID */}
                    <Text style={styles.departmentId}>
                        Email ID: {total.selectedEmployee.email ? total.selectedEmployee.email : "Not present"}
                    </Text>
                </View>
                {/* ConfirmButton */}
